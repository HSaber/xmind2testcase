...
"""
Convert XMind fie to feishu testcase xlsx file 

feishu official document about import XLSX testcase file: https://www.feishu.net/book/feishupmshelp/243.mhtml 
"""


def xmind_to_feishu_xlsx_file(xmind_file):
    ...
    fileheader = ["测试用例类型(template)", "用例名称(name)", "业务线(business)", "描述(description)", "前置条件(field_f717b4)",
                  "执行步骤(field_023f96)", "预期结果(field_2c7371)", "附件(field_603db5)", "标签(field_65e1cc)",
                  "用例分级(field_ad0ad4)",
                  "关联需求(field_d73be1)", "用例类型(field_e42a97)", "创建人(owner)", "优先级(priority)", "关注人(watchers)",
                  "拉群方式选择(group_type)",
                  "流程角色(role_owners)", "当前状态(work_item_status)", "自增数字(auto_number)", "是否冻结(is_frozen)",
                  "当前状态开始时间(status_begin_time)"]

    feishu_testcase_rows = [fileheader]
    feishu_testcase_rows.append(
        ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '负责人(test_case_owner)', '', '', '', ''])
    for testcase in testcases:
        row = gen_a_testcase_row(testcase)
        feishu_testcase_rows.append(row)

    feishu_file = xmind_file[:-6] + '.xlsx'
    ...
    ws = wb.active
    # 设置工作表名称
    ws.title = '测试用例(test_cases)'
    # 写入表头ws.append(fileheader)
    # 写入数据
    for row in feishu_testcase_rows:
        ws.append(row)
...
def gen_a_testcase_row(testcase_dict):
    ...
    row = [case_jobtype, case_title, case_business, case_description, case_precontion,
           ...
           case_priority, case_owner, case_group, case_owner, case_status, case_auto_number, case_frozen,
           case_begin_time]
...
if __name__ == '__main__':
    ...
    print('Conver the xmind file to a feishu xlsx file succssfully: %s', feishu_xlsx_file)